...
public abstract class ShaderProgram {
    ...
    private static final String LOCATION = "/engine/ui/gui/renderer/shader/";
    ...
    protected void loadLocation(String name) { uniforms.put(name,GL20.glGetUniformLocation(program,name)); }
    ...
    public void cleanUp() { GL20.glDeleteProgram(program); }

    private static float[] mat4x4 = new float[16];
    protected void loadMat4(String name, Matrix4f mat) { GL20.glUniformMatrix4fv(getLocation(name),false,mat.get(mat4x4)); }

    protected void loadVec4f(String name, Vector4f vec) { loadVec4f(name,vec.x,vec.y,vec.z,vec.w); }
    protected void loadVec3f(String name, Vector3f vec) { loadVec3f(name,vec.x,vec.y,vec.z); }
    protected void loadVec2f(String name, Vector2f vec) { loadVec2f(name,vec.x,vec.y); }

    protected void loadVec4f(String name, float x, float y, float z, float w) { GL20.glUniform4f(getLocation(name),x,y,z,w); }
    protected void loadVec3f(String name, float x, float y, float z) { GL20.glUniform3f(getLocation(name),x,y,z); }
    protected void loadVec2f(String name, float x, float y) { GL20.glUniform2f(getLocation(name),x,y); }

    protected void loadFloat(String name, float val) { GL20.glUniform1f(getLocation(name),val); }
    protected void loadInt(String name, int val) { GL20.glUniform1i(getLocation(name),val); }
