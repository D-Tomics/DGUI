...
public enum CellSize {
    ...
    protected float getCellWidth(GridConstraint constraint, float cellWidth, D_Gui component) {
        float width = (component.getStyle().getWidth() + component.getStyle().getMarginWidth()) / (constraint.w() + 1);
        switch (this) {
            case COMPACT:
            case COMPACT_WIDTH:
                return width;
    ...
    protected float getHeight(GridConstraint constraint, float cellHeight, D_Gui component) {
        float height = (component.getStyle().getHeight() + component.getStyle().getMarginHeight()) / (constraint.h() + 1);
        switch (this) {
            case COMPACT:
            case COMPACT_HEIGHT:
                return height;
